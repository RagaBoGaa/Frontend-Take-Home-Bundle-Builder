...
- **Accordion animation:** Uses `max-height` CSS transition — a well-known hack that causes a perceptible delay on collapse because the browser animates the full declared `max-height` value regardless of actual content size. The correct solution is a `ResizeObserver` measuring actual content height, then animating to/from that explicit pixel value.
- **Testing:** No unit or integration tests — would add React Testing Library + Vitest for production coverage of the reducer, `enforceBusinessRules`, and `buildReviewLineItems`.
- **SSR compatibility:** The `Icon` component uses `DOMParser` which is browser-only. In a Next.js / SSR context, the icon injection would need a `useEffect` guard (already present) and a server-side fallback.
- **Image optimisation pipeline:** WebP files are currently manually exported. A production setup would add `sharp` or Vite's `vite-plugin-image-optimizer` to automate compression at build time.
- **Font self-hosting:** Google Fonts introduces a third-party network request. For optimal performance and privacy compliance (GDPR), Plus Jakarta Sans should be self-hosted via `@font-face` in production.
